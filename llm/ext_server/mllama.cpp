...
static std::string format(const char *fmt, ...) {
    ...
    int size2 = vsnprintf(b.data(), n + 1, fmt, args);
    REQUIRE(size2 == n);
    va_end(args);
...
static std::vector<bool> get_bool_array(const gguf_context *ctx, const std::string &key) {
    const int i = get_key_index(ctx, key.c_str());
    const int n = gguf_get_arr_n(ctx, i);
    const bool *data = (bool *)gguf_get_arr_data(ctx, i);
...
static ggml_tensor *mllama_image_build_encoder_layer(
    ...
    const std::vector<bool> &intermediate_layers, struct ggml_tensor **intermediate_embeddings) {
    ...
    if (intermediate_embeddings != nullptr && intermediate_layers[il]) {
...
static ggml_cgraph *mllama_image_build_graph(mllama_ctx *ctx, const mllama_image_batch *imgs) {
    ...
    int num_channels = imgs->data[0].num_channels;
    if (num_channels == 0) {
        num_channels = 3;
    }

    int num_tiles = imgs->data[0].num_tiles;
    if (num_tiles == 0) {
        num_tiles = hparams.n_tiles;
    }
    ...
    if (model.pre_ln_w != nullptr) {
        embeddings = ggml_norm(ctx0, embeddings, hparams.eps);
    ...
    struct ggml_tensor *intermediate_embeddings = ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, 0, (num_positions + num_padding_patches) * num_tiles);
    ...
    for (size_t il = 0; il < model.layers.size(); il++) {
        embeddings = mllama_image_build_encoder_layer(
            ...
            hparams.intermediate_layers, &intermediate_embeddings);
    }
    ...
    for (size_t il = 0; il < model.global_layers.size(); il++) {
        ...
        embeddings = mllama_image_build_encoder_layer(
            ctx0, il, layer, embeddings,
            hparams.eps, hidden_size, batch_size, n_head, d_head,
            hparams.intermediate_layers, nullptr);
    }
...
static std::vector<struct mllama_layer> mllama_layers_load(struct ggml_context *ctx, const char *prefix, const int n) {
    ...
    for (size_t i = 0; i < layers.size(); i++) {
        ...
        layer.gate = mllama_tensor_load(ctx, format("%s.blk.%d.attn_gate", prefix, i).c_str(), true);
...
struct mllama_ctx *mllama_model_load(const char *fname, const int verbosity = 1) {
    ...
    hparams.intermediate_layers = get_bool_array(ctx, "mllama.vision.intermediate_layers");
...
bool mllama_image_batch_encode(mllama_ctx *ctx, const int n_threads, const mllama_image_batch *imgs, float *vec) {
...

    {
        ...
        for (size_t i = 0; i < imgs->size; i++) {
            const int width = imgs->data[i].width;
            const int height = imgs->data[i].height;
            const int num_channels = imgs->data[i].num_channels;
            const int num_tiles = imgs->data[i].num_tiles;
                    ...
                    for (int k = 0; k < num_channels; k++) {
                        for (int y = 0; y < height; y++) {
                            for (int x = 0; x < width; x++) {
                                data[(b * num_tiles * num_channels * height * width) + (t * num_channels * height * width) + (k * height * width) + (y * width) + x] = imgs->data[i].data[(t * num_channels * height * width) + (k * height * width) + (y * width) + x];
                            }
                        }
                    }
                }
...

    {
        ...
        if (aspect_ratios != nullptr) {
            ...
            for (size_t i = 0; i < imgs->size; i++) {
                aspect_ratios_data[0] = imgs->data[i].aspect_ratio_id;
            }
